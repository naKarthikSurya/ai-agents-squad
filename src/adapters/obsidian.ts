...
function getRepoInfo(projectRoot: string): RepoInfo {
  const info: RepoInfo = {
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      if (deps['@angular/core']) info.techStack.push('Angular');
      if (deps['express']) info.techStack.push('Express');
      if (deps['@nestjs/core']) info.techStack.push('NestJS');
      if (deps['fastify']) info.techStack.push('Fastify');
      if (deps['prisma']) info.techStack.push('Prisma ORM');
...
function buildDirTree(dir: string, baseDir: string, depth: number = 0): string {
  if (depth > 3) return ''; // limit depth to 3 for performance
  let result = '';
  try {
    ...
    for (const file of files) {
      if (file.name.startsWith('.') && file.name !== '.talos') continue;
      if (['node_modules', 'dist', 'build', 'out', 'package-lock.json', 'yarn.lock', 'pnpm-lock.yaml'].includes(file.name)) continue;
